...
if not TOKEN:
    raise RuntimeError(
        "DISCORD_BOT_TOKEN is missing. Add it as an environment variable."
    )
...
async def route(ctx: commands.Context):
    """Show available transportation locations."""
...
async def drive(ctx: commands.Context, destination=None):
    """Drive from the current location to another location."""
...
async def vehicle_error(ctx: commands.Context, error):
    await ctx.send(f"❌ Vehicle error: `{error}`")
    print(f"Vehicle error: {error}")
